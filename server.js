    ...
    mongoClient.connect("mongodb://localhost:27017", {
        ...
        app.get("/Search", async function (request, result) {
            ...
            if (request.session.user) {
                ...
                let fileUploaded = await recursiveSearch(user.uploaded, search);
                let fileShared = await recursiveSearchShared(user.sharedWithMe, search);
        ...
        app.get("/Blog", async function (request, result) {
            let usersList = await database.collection("users").find({}).toArray();
        ...
        app.post("/Register", async function (request, result) {
            ...
            var email = request.fields.email;
            var password = request.fields.password;
            var wallet = request.fields.wallet;
            var reset_token = "";
            var isVerified = true;
        ...
        //buy Item
        // app.post("/addAuthor", function(request, result){
